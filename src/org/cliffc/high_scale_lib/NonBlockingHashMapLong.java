 ...
 * multi-thread safe, including all update operations. null {@link
 ...
 * concurrency factor. Scaling is linear up to 768 CPUs on a 768-CPU Azul box,
 * even with 100% updates or 100% reads or any fraction in-between. Linear
 * scaling up to all cpus has been observed on a 32-way Sun US2 box, 32-way Sun
 * Niagra box, 8-way Intel box and a 4-way Power box.
 *
 * <p><strong>The main benefit of this class</strong> over using plain {@link
 * org.cliffc.high_scale_lib.NonBlockingHashMap} with {@link Long} keys is that
 * it avoids the auto-boxing and unboxing costs. Since auto-boxing is
 ...
 * java.util.Hashtable}, and includes versions of methods corresponding to each
 * method of <tt>Hashtable</tt>. However, even though all operations are
 ...
 * terribly expensive, but it is not free either; during resize operations table
 ...
 *
 * <p> Like {@link Hashtable} but unlike {@link HashMap}, this class does
 * <em>not</em> allow <tt>null</tt> to be used as a value.
 *
 *
 * @since 1.5
 * @author Cliff Click
...
public class NonBlockingHashMapLong<TypeV>
     ...
     * tradeoff. {@code true} optimizes for space and is the default. null     {@code
   *  false} optimizes for speed and doubles space costs for roughly a 10%
     ...
     * the space-for-speed tradeoff. {@code true} optimizes for space and is the
     * default. {@code false} optimizes for speed and doubles space costs for
     * roughly a 10% speed improvement.
     */
    ...
    /**
     * Atomically do a {@link #remove(long)} if-and-only-if the key is mapped to
     * a value which is
    ...
    /**
     * Atomically do a
     * <code>put(key,val)</code> if-and-only-if the key is mapped to some value
     * already.
     *
     * @throws NullPointerException if the specified value is null
     */
    ...
    /**
     * Atomically do a
     * <code>put(key,newValue)</code> if-and-only-if the key is mapped a value
     * which is
     * <code>equals</code> to
     * <code>oldValue</code>.
     *
     ...
     * if this map contains no mapping for the key. <p>More formally, if this
     * map contains a mapping from a key {@code k} to a value {@code v} such
     * that {@code key==k}, then this method returns {@code v}; otherwise it
    ...
    /**
     * Returns an enumeration of the <strong>auto-boxed</strong> keys in this
     * table. <strong>Warning:</strong> this version will auto-box all returned
     * keys.
     *
     ...
     * iteration. The {@link org.cliffc.high_scale_lib.NonBlockingHashMap} does
     * not normally create or using {@link java.util.Map.Entry} objects so they
     * will be created soley to support this iteration. Iterating using
