 ...
 * identity equality. Uses System.identityHashCode() to calculate hashMap. Key
...
public class NonBlockingIdentityHashMap<TypeK, TypeV>
     ...
     * nor value can be null. <p> The value can be retrieved by calling
     * {@link #get} with a key that is equal to the original key.
     *
     ...
     * (<tt>"{}"</tt>). Adjacent mappings are separated by the characters <tt>",
     * "</tt> (comma and space). Each key-value mapping is rendered as the key
     * followed by an equals sign (<tt>"="</tt>) followed by the associated
     ...
     * that {@code key.equals(k)}, then this method returns {@code v}; otherwise
     ...
     * create or using {@link java.util.Map.Entry} objects so they will be
     * created soley to support this iteration. Iterating using {@link #keySet}
     * or {@link
