 ...
 * operations. Updates are done with CAS's to no particular table element. The
 * intent is to support highly scalable counters, r/w locks, and other
 * structures where the updates are associative, loss-free (no-brainer), and
 * otherwise happen at such a high volume that the cache contention for CAS'ing
 * a single word is unacceptable.
 *
 * <p>This API is overkill for simple counters (e.g. no need for the 'mask') and
 * is untested as an API for making a scalable r/w lock and so is likely to
 * change!
 *
 * @since 1.5
 * @author Cliff Click
...
public class ConcurrentAutoTable implements Serializable {
    ...
    // --- public interface ---
    /**
     * Add the given value to current counter value. Concurrent updates will not
     * be lost, but addAndGet or getAndAdd are not implemented because the total
     * counter value (i.e., {@link #get}) is not atomically updated. Updates are
     * striped across an array of counters to avoid cache contention and has
     * been tested with performance scaling linearly up to 768 CPUs.
     */
    ...
    /**
     * Atomically set the sum of the striped counters to specified value. Rather
     * more expensive than a simple store, in order to remain atomic.
     */
