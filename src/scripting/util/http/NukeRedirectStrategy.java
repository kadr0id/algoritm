...
public class NukeRedirectStrategy implements RedirectStrategy {
    ...
    public NukeRedirectStrategy() {
        super();
    }

    public boolean isRedirectedByRefreshHeader(final HttpRequest request,
        ...
        Header refreshHeader = response.getFirstHeader("refresh");
        if (refreshHeader != null) {
            String[] match = TextUtility.ExtractWhole(refreshHeader.getValue(), "[0-9]*\\s*;\\s*(URL|url)=[\"']*([^\"']*)[\"']*");
    ...
    public boolean isRedirected(
        ...
        switch (statusCode) {
            ...
            case HttpStatus.SC_SEE_OTHER:
                return true;
            default:
                if (isRedirectedByRefreshHeader(request, response, context)) {
                    return true;
                } else {
                    return false;
                }
    ...
    public URI getLocationURI(
        ...
        if (Strings.isNullOrEmpty(uri.getPath())) {
            location = location + "/";
            uri = createLocationURI(location);
        }
        ...
        } else if (redirectLocations.getAll().size() >= 10) {
            throw new CircularRedirectException("Circular redirect to '" + uri + "'");
        }
