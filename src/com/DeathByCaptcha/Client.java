...
abstract public class Client {
    ...
    protected HttpRequestBase currentReq;
     ...
     */
    abstract public User getUser() throws IOException, com.DeathByCaptcha.Exception, URISyntaxException, InterruptedException, TaskStoppedException, SubmissionTimeoutException;
    ...
    public Captcha upload(File f)
            throws IOException, FileNotFoundException, com.DeathByCaptcha.Exception, URISyntaxException, InterruptedException, TaskStoppedException, SubmissionTimeoutException {
    ...
    /**
     * Tries to solve a CAPTCHA by uploading it and polling for its status and
     * text with arbitrary timeout.
     *
     * @param img CAPTCHA image byte vector
     * @param timeout Solving timeout (in seconds)
     * @return CAPTCHA object if uploaded and correctly solved, null otherwise
     */
    public Captcha decode(byte[] img, int timeout) throws IOException, Exception, InterruptedException, URISyntaxException, InterruptedException, TaskStoppedException, SubmissionTimeoutException {
        ...
        if (null != captcha) {
            while (deadline > System.currentTimeMillis() && !captcha.isSolved()) {
                if (this instanceof CancellableHttpClient) {
                    boolean waitResult = (((CancellableHttpClient) this).getStopEvent()).waitOne(Client.POLLS_INTERVAL, TimeUnit.MILLISECONDS);
                    if (waitResult == true) {
                        if ((((CancellableHttpClient) this).getStopEvent()).getSource().equals("cancelled")) {
    ...
    public Captcha decode(byte[] img)
            throws IOException, com.DeathByCaptcha.Exception, InterruptedException, URISyntaxException, InterruptedException, TaskStoppedException, SubmissionTimeoutException {
    ...
    public Captcha decode(File f, int timeout)
            throws IOException, FileNotFoundException, com.DeathByCaptcha.Exception, InterruptedException, URISyntaxException, InterruptedException, TaskStoppedException, SubmissionTimeoutException {
